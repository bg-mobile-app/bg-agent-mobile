...
class PayoutRequestItem {
  ...
  factory PayoutRequestItem.fromJson(Map<String, dynamic> json) {
    ...
    return PayoutRequestItem(
      ...
      bookingId: json['booking']?.toString() ?? json['bookingId']?.toString() ?? json['booking_id']?.toString() ?? '-',
      ...
      processingBy: json['processingBy']?.toString() ?? json['processing_by']?.toString() ?? '-',
      referenceBy: json['referenceBy']?.toString() ?? json['reference_by']?.toString() ?? '-',
      ...
      currentRequest: json['currentRequest']?.toString() ?? json['current_request']?.toString() ?? json['status']?.toString() ?? '-',
    );
  }
}

class PayoutRequestService {
  PayoutRequestService({ApiClient? apiClient}) : _apiClient = apiClient ?? ApiClient();

  final ApiClient _apiClient;

  Future<PayoutRequestPage> getRequests({String? status, String? search, String? branch, int page = 1}) async {
    ...
    final response = await _apiClient.get('/payment/agency/payout-request/', queryParameters: query);
    ...
    return PayoutRequestPage(
      ...
      results: rawResults.whereType<Map>().map((e) => PayoutRequestItem.fromJson(Map<String, dynamic>.from(e))).toList(),
    );
  }
}
